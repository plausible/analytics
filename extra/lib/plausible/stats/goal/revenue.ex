defmodule Plausible.Stats.Goal.Revenue do
  ...
  """
  def preload_revenue_currencies(site, goals, metrics, dimensions, remove_unavailable_revenue_metrics) do
    cond do
      not requested?(metrics) ->
        {:ok, %{}}

      length(goals) > 0 and not available?(site) and remove_unavailable_revenue_metrics ->
        {:ok, %{}}

      length(goals) > 0 and not available?(site) and not remove_unavailable_revenue_metrics ->
        {:error, "The this site does not have access to the revenue metrics feature."}

      true ->
        calculate_revenue_currencies(goals, dimensions, remove_unavailable_revenue_metrics)
    end
  end

  defp calculate_revenue_currencies(goals, dimensions, remove_unavailable_revenue_metrics) do
    goal_currency_map =
      goals
    ...
    case {currencies, goal_dimension?, remove_unavailable_revenue_metrics} do
      {[currency], false, _} -> {:ok, %{default: currency}}
      {_, true, _} -> {:ok, goal_currency_map}
      {_, _, true} -> {:ok, %{}}
      _ -> {:error, "To use revenue metrics, all filtered goals must have the same revenue tracking currency configured in Plausible."}
    end
  end
