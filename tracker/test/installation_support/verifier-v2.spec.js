...
import { verifyV2 as verify } from '../support/installation-support-playwright-wrappers'
...
async function mockEventResponseSuccess(page, responseDelay = 0) {
  await page.context().route('https://plausible.io/api/event', async (route) => {
    if (responseDelay > 0) {
...
test.describe('v2 verifier (basic diagnostics)', () => {
  test('plausible-web installed with recommended snippet', async ({ page }, { testId }) => {
    ...
    const result = await verify(page, {url: url, expectedDataDomain: false})
  ...
  test('no snippet found', async ({ page }, { testId }) => {
    const { url } = await initializePageDynamically(page, {
      testId,
      scriptConfig: ''
    })
...
test.describe('v1 verifier (logging)', () => {
  test('console logs in debug mode', async ({ page }, { testId }) => {
    ...
    await verify(page, {url: url, expectedDataDomain: false, debug: true})
  ...
  test('does not log by default', async ({ page }, { testId }) => {
    ...
    await verify(page, {url: url, expectedDataDomain: false})
