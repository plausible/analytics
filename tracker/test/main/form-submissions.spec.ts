 ...
 * forms submitted before the tracker script attaches the event listener will _not_ be tracked. 
 */
...
test.describe('trackFormSubmissions is enabled', () => {
  ...
  test('tracks forms that use GET method', async ({ page, browserName }, {
    ...
    await initializePageDynamically(page, {
      ...
      <div>
        <form method="GET" ${
          // this test (and therefore feature) is flaky on Firefox unless the navigation from submitting the form is slow enough for the track request to be attempted
          browserName === 'firefox'
