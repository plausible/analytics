...
test.describe(`transformRequest examples from /docs work`, () => {
  ...
  test('you can omit automatically tracked url property from tagged link clicks', async ({
    ...
    const { url } = await initializePageDynamically(page, {
      testId,
      scriptConfig: config,
      bodyContent: `<a class="plausible-event-name=Purchase plausible-event-discounted=true" href="https://example.com/target?user=sensitive">Purchase</a>`
    })
  ...
  test('you can track pages using their canonical url', async ({ page }, {
    ...
    const { url } = await initializePageDynamically(page, {
      ...
      scriptConfig: `
        ...
        <script type="module">import { init, track } from '/tracker/js/npm_package/plausible.js'; init(${serializeWithFunctions(
          {
            ...DEFAULT_CONFIG,
