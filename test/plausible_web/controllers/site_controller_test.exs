defmodule PlausibleWeb.SiteControllerTest do
  ...
  describe "domain change v2" do
    setup [:create_user, :log_in, :create_site]

    test "domain change v2 form renders", %{conn: conn, site: site} do
      conn = get(conn, Routes.site_path(conn, :change_domain_v2, site.domain))
      resp = html_response(conn, 200)
      assert resp =~ Routes.site_path(conn, :change_domain_v2_submit, site.domain)

      assert text(resp) =~
               "Once you change your domain, you must update Plausible Installation on your site within 72 hours"
    end

    test "domain change v2 form submission when no change is made", %{conn: conn, site: site} do
      conn =
        put(conn, Routes.site_path(conn, :change_domain_v2_submit, site.domain), %{
          "site" => %{"domain" => site.domain}
        })
    ...
    test "domain change v2 form submission to an existing domain", %{conn: conn, site: site} do
      another_site = insert(:site)
    ...
    test "domain change v2 form submission to a domain in transition period", %{
      conn: conn,
      site: site
    } do
      another_site = insert(:site, domain_changed_from: "foo.example.com")
    ...
    test "domain change v2 successful form submission redirects to installation", %{
      conn: conn,
      site: site
    } do
      original_domain = site.domain
