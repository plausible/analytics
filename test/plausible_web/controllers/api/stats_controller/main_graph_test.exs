defmodule PlausibleWeb.Api.StatsController.MainGraphTest do
  ...
  describe "GET /api/stats/main-graph - plot" do
    ...
    test "displays visitors for a day", %{conn: conn, site: site} do
      insert(:pageview, site_id: site.domain, timestamp: ~N[2019-01-01 00:00:00])
      insert(:pageview, site_id: site.domain, timestamp: ~N[2019-01-01 23:59:00])
    ...
    test "displays hourly stats in configured timezone", %{conn: conn, user: user} do
      ...
      insert(:pageview, site_id: site.domain, timestamp: ~N[2019-01-01 00:00:00]) # Timestamp is in UTC
    ...
    test "displays visitors for a month", %{conn: conn, site: site} do
      insert(:pageview, site_id: site.domain, timestamp: ~N[2019-01-01 12:00:00])
      insert(:pageview, site_id: site.domain, timestamp: ~N[2019-01-31 12:00:00])
    ...
    test "displays visitors for 6 months", %{conn: conn, site: site} do
      insert(:pageview, site_id: site.domain)
      insert(:pageview, site_id: site.domain, timestamp: months_ago(5))
  ...
  describe "GET /api/stats/main-graph - top stats" do
    ...
    test "unique users counts distinct user ids", %{conn: conn, site: site} do
      insert(:pageview, site_id: site.domain, user_id: @user_id, timestamp: ~N[2019-01-01 00:00:00])
      insert(:pageview, site_id: site.domain, user_id: @user_id, timestamp: ~N[2019-01-01 23:59:00])
    ...
    test "does not count custom events in custom user ids", %{conn: conn, site: site} do
      insert(:pageview, site_id: site.domain, timestamp: ~N[2019-01-01 00:00:00])
      insert(:event, name: "Custom", site_id: site.domain, timestamp: ~N[2019-01-01 00:00:00])
    ...
    test "compares pageviews with previous time period", %{conn: conn, site: site} do
      insert(:pageview, site_id: site.domain, timestamp: ~N[2019-01-01 01:00:00])
      insert(:pageview, site_id: site.domain, timestamp: ~N[2019-01-01 02:00:00])

      insert(:pageview, site_id: site.domain, timestamp: ~N[2019-01-02 02:00:00])
    ...
    test "calculates bounce rate", %{conn: conn, site: site} do
      insert(:session, site_id: site.domain, is_bounce: true, start: ~N[2019-01-01 01:00:00])
      insert(:session, site_id: site.domain, is_bounce: false, start: ~N[2019-01-01 02:00:00])
    ...
    test "calculates change in bounce rate", %{conn: conn, site: site} do
      ...
      insert(:session, site_id: site.domain, is_bounce: true, start: ~N[2019-01-02 01:00:00])
      insert(:session, site_id: site.domain, is_bounce: true, start: ~N[2019-01-02 01:00:00])
      insert(:session, site_id: site.domain, is_bounce: false, start: ~N[2019-01-02 02:00:00])
  ...
  describe "GET /api/stats/main-graph - filtered for goal" do
    ...
    test "returns total unique visitors", %{conn: conn, site: site} do
      insert(:pageview, site_id: site.domain, timestamp: ~N[2019-01-01 02:00:00])
      insert(:pageview, site_id: site.domain, user_id: @user_id, timestamp: ~N[2019-01-01 01:00:00])
      insert(:event, name: "Signup", site_id: site.domain, user_id: @user_id, timestamp: ~N[2019-01-01 02:00:00])
