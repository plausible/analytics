defmodule PlausibleWeb.Api.SessionDurationTest do
  ...
  test "PV -> 10min -> enrich -> 10min -> new PV -> visit_duration is 20min", %{conn: conn, site: site} do
    params = %{@pv_params | domain: site.domain}

    event_id = send_pageview(params) |> response(202)
    send_enrich(event_id, ~N[2022-01-01 00:10:00])

    send_pageview(%{params | url: "http://some.url/anotherpage", timestamp: ~N[2022-01-01 00:20:00]})
    Plausible.Session.WriteBuffer.flush()
  ...
  test "event with timestamp before session.timestamp will not create/update session", %{conn: conn, site: site} do
    params = %{@pv_params | domain: site.domain}

    send_pageview(%{params | url: "http://some.url", timestamp: ~N[2022-01-01 00:00:01]})
    send_pageview(%{params | url: "http://some.url/anotherpage", timestamp: ~N[2022-01-01 00:00:00]})
    Plausible.Session.WriteBuffer.flush()
