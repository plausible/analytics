defmodule Plausible.SitesTest do
  ...
  describe "stats_range" do
    test "is nil for both dates if site has no stats" do
      site = new_site()

      assert Sites.stats_range(site) == %{stats_start_date: nil, stats_end_date: nil}
    end

    test "reflects native stats start and end dates when there's no import" do
      site = new_site()
    ...
    test "reflects import start and end dates when there are no native stats" do
      site = new_site()
      insert(:site_import, site: site, start_date: ~D[2019-03-01], end_date: ~D[2019-04-01])

      assert Sites.stats_range(site) == %{
               stats_start_date: ~D[2019-03-01],
               stats_end_date: ~D[2019-04-01]
             }
    end

    test "extends the range with imported stats falling outside of native stats" do
      site = new_site()
    ...
    test "ignores imports that are not completed" do
      site = new_site()

      insert(:site_import,
        site: site,
        start_date: ~D[2010-01-01],
        end_date: ~D[2010-06-01],
        status: :importing
      )
