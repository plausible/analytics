defmodule Plausible.Stats.QueryComparisonsTest do
  ...
  test "timeseries last 28d period compares the same period with and without match_day_of_week=true",
    ...
    assert %Stats.QueryResult{results: results1, meta: meta1} = Stats.query(site, query1)
    assert %Stats.QueryResult{results: results2, meta: meta2} = Stats.query(site, query2)
    ...
    assert List.last(meta1[:time_labels]) == expected_last_date
    assert List.first(meta1[:comparison_time_labels]) == expected_comparison_first_date
    assert List.last(meta1[:comparison_time_labels]) == expected_comparison_last_date
  end
  ...
  test "timeseries last 91d period in year_over_year comparison", %{
    ...
    assert length(time_labels) == 91
    assert length(comparison_time_labels) == 91
