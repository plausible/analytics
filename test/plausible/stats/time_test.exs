defmodule Plausible.Stats.TimeTest do
  ...
  describe "extra_time_labels/2" do
    test "returns empty list when n is 0" do
      ...
      assert extra_time_labels(query, 0) == []
    end
    ...
    test "with time:day dimension" do
      ...
      assert List.last(time_labels(query)) == "2022-01-03"
      assert extra_time_labels(query, 3) == ["2022-01-04", "2022-01-05", "2022-01-06"]
    end

    test "with time:week dimension" do
      query = %{
        dimensions: ["time:week"],
        utc_time_range: DateTimeRange.new!(~D[2022-01-03], ~D[2022-01-16], "UTC"),
        timezone: "UTC"
      }

      assert List.last(time_labels(query)) == "2022-01-10"
      assert extra_time_labels(query, 2) == ["2022-01-17", "2022-01-24"]
    end

    test "with time:month dimension" do
      query = %{
        dimensions: ["time:month"],
        utc_time_range: DateTimeRange.new!(~D[2022-01-01], ~D[2022-03-31], "UTC"),
        timezone: "UTC"
      }

      assert List.last(time_labels(query)) == "2022-03-01"
      assert extra_time_labels(query, 2) == ["2022-04-01", "2022-05-01"]
    end
    ...
    test "with time:hour dimension" do
      ...
      assert List.last(time_labels(query)) == "2022-01-01 23:00:00"
      assert extra_time_labels(query, 2) == ["2022-01-02 00:00:00", "2022-01-02 01:00:00"]
    end

    test "with time:minute dimension" do
      query = %{
        dimensions: ["time:minute"],
        now: ~U[2022-01-01 12:05:00Z],
        utc_time_range: DateTimeRange.new!(~U[2022-01-01 12:00:00Z], ~U[2022-01-01 12:10:00Z]),
        timezone: "UTC"
      }

      # time_labels stops at 12:04:00 (due to now = 12:05:00)
      assert List.last(time_labels(query)) == "2022-01-01 12:04:00"
      assert extra_time_labels(query, 2) == ["2022-01-01 12:05:00", "2022-01-01 12:06:00"]
    end
  end
end
