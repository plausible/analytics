defmodule Plausible.OAuthTest do
  ...
  describe "normalize_scopes/1" do
    @all ["stats:read:*", "sites:read:*"]

    test "defaults to all supported scopes when absent, empty or blank" do
      assert OAuth.normalize_scopes(nil) == {:ok, @all}
      assert OAuth.normalize_scopes("") == {:ok, @all}
      assert OAuth.normalize_scopes("   ") == {:ok, @all}
    end

    test "accepts a subset of supported scopes in canonical order" do
      assert OAuth.normalize_scopes("sites:read:*") == {:ok, ["sites:read:*"]}
      assert OAuth.normalize_scopes("sites:read:* stats:read:*") == {:ok, @all}
    end

    test "rejects the whole request if any scope is unsupported" do
      assert OAuth.normalize_scopes("stats:write:*") == {:error, :invalid_scope}
      assert OAuth.normalize_scopes("bogus other") == {:error, :invalid_scope}
      assert OAuth.normalize_scopes("stats:read:* bogus:scope") == {:error, :invalid_scope}
    end
  end
