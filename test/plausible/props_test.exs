defmodule Plausible.PropsTest do
  ...
  test "auto_import/1 saves the most frequent prop keys" do
    site = insert(:site)
    ...
    {:ok, site} = Plausible.Props.auto_import(site)

    assert %Plausible.Site{allowed_event_props: ["amount", "logged_in", "is_customer"]} =
             Plausible.Repo.reload!(site)
  end

  test "auto_import/1 skips invalid keys" do
    site = insert(:site)
  ...
  test "auto_import/1 can be run multiple times" do
    site = insert(:site)
    ...
    {:ok, %Plausible.Site{allowed_event_props: ["amount", "logged_in", "is_customer"]}} =
      Plausible.Props.auto_import(site)
