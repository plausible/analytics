defmodule Plausible.Sites do
    ...
    with :ok <- Plausible.Teams.Adapter.Read.Billing.ensure_can_add_new_site(user) do
      ...
      |> Ecto.Multi.run(:clear_changed_from, fn
        _repo, %{site_changeset: %{changes: %{domain: domain}}} ->
          case get_for_user(user, domain, [:owner]) do
  ...
  @spec get_for_user!(Auth.User.t(), String.t(), [:super_admin | :owner | :admin | :viewer]) ::
          Site.t()
  def get_for_user!(user, domain, roles \\ [:owner, :admin, :viewer])

  def get_for_user!(user, domain, roles) do
    if :super_admin in roles and Auth.is_super_admin?(user.id) do
