defmodule Plausible.Stats do
    ...
    groups =
      Repo.all(
        from e in base_query(site, %{query | filters: %{}}),
          ...
          select:
            {fragment(
               "date_trunc('month', ? at time zone 'utc' at time zone ?)",
               e.timestamp,
               ^site.timezone
             ), count(e.fingerprint, :distinct)}
      )
      |> Enum.into(%{})
  ...
  def calculate_plot(site, %Query{step_type: "hour"} = query) do
    ...
    groups =
      Repo.all(
        from e in base_query(site, %{query | filters: %{}}),
          ...
          select:
            {fragment(
               "date_trunc('hour', ? at time zone 'utc' at time zone ?)",
               e.timestamp,
               ^site.timezone
             ), count(e.fingerprint, :distinct)}
      )
    ...
    present_index =
      Enum.find_index(steps, fn step ->
        step == Timex.now(site.timezone) |> truncate_to_hour |> NaiveDateTime.truncate(:second)
      end)
  ...
  def top_referrers_for_goal(site, query, limit \\ 5) do
    Repo.all(
      from e in base_query(site, query),
        select: %{
          name: e.initial_referrer_source,
          url: min(e.initial_referrer),
          count: count(e.fingerprint, :distinct)
        },
  ...
  def top_referrers(site, query, limit \\ 5, include \\ []) do
    ...
    if "bounce_rate" in include do
      bounce_rates =
        bounce_rates_by_referrer_source(
          site,
          query,
          Enum.map(referrers, fn ref -> ref[:name] end)
        )
  ...
  def referrer_drilldown(site, query, referrer, include \\ []) do
    ...
    referring_urls =
      if "bounce_rate" in include do
        bounce_rates =
          bounce_rates_by_referring_url(
            site,
            query,
            Enum.map(referring_urls, fn ref -> ref[:name] end)
          )
  ...
  def top_pages(site, query, limit \\ 5, include \\ []) do
    ...
    if "bounce_rate" in include do
      bounce_rates =
        bounce_rates_by_page_url(site, query, Enum.map(pages, fn page -> page[:name] end))
    ...
    if Enum.count(pages) > 0 do
      Repo.all(
        from e in base_query(site, query),
          ...
          group_by: e.pathname,
          select: %{
            name: fragment("concat('Visit ', ?)", e.pathname),
            count: count(e.fingerprint, :distinct)
          }
      )
    else
  ...
  defp truncate_to_hour(datetime) do
    {:ok, datetime} =
      NaiveDateTime.new(datetime.year, datetime.month, datetime.day, datetime.hour, 0, 0, 0)
