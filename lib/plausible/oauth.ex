defmodule Plausible.OAuth do
  ...
  An empty/absent request defaults to all supported scopes (RFC 6749 §3.3
  pre-defined default). A non-empty request must name only supported scopes;
  if it names any scope we don't support, the whole request is rejected with
  `{:error, :invalid_scope}` (RFC 6749 §4.1.2.1) rather than silently dropping
  the unsupported entries. The returned scopes follow the canonical
  `supported_scopes/0` order regardless of the request order.
  """
  @spec normalize_scopes(String.t() | nil) :: {:ok, [String.t()]} | {:error, :invalid_scope}
  def normalize_scopes(scope) do
    requested = (scope || "") |> String.split(" ", trim: true) |> MapSet.new()
    supported = MapSet.new(@supported_scopes)

    cond do
      MapSet.size(requested) == 0 -> {:ok, @supported_scopes}
      not MapSet.subset?(requested, supported) -> {:error, :invalid_scope}
      true -> {:ok, Enum.filter(@supported_scopes, &MapSet.member?(requested, &1))}
    end
  end
