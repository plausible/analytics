defmodule Plausible.Teams.Sites do
  ...
  def list_with_clickhouse(user, team, opts \\ []) do
    ...
    {relative_start_date, relative_end_date} =
      calculate_relative_dates(date_range, DateTime.utc_now())

    earliest_possible_start_datetime = DateTime.from_naive!(relative_start_date, "Etc/UTC-12")
    latest_possible_end_datetime = DateTime.from_naive!(relative_end_date, "Etc/UTC+14")
    ...
    clickhouse_query =
      from(e in Plausible.ClickhouseEventV2,
        ...
        where:
          e.timestamp >= ^earliest_possible_start_datetime and
            e.timestamp <= ^latest_possible_end_datetime and
            fragment(
              """
              ? >= parseDateTime(formatDateTime(?, '%F %T', ?))
              AND ? <= parseDateTime(formatDateTime(?, '%F %T', ?))
              """,
              e.timestamp,
              ^relative_start_date,
              sites.timezone,
  ...
  defp calculate_relative_dates({:last_n_days, n}, now) do
    ...
    {NaiveDateTime.new!(start_date, ~T[00:00:00]), NaiveDateTime.new!(end_date, ~T[23:59:00])}
  end
