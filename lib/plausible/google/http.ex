defmodule Plausible.Google.HTTP do
  ...
  @spec get_ga4_report(Plausible.Google.GA4.ReportRequest.t()) ::
          {:ok, {[map()], String.t() | nil}} | {:error, any()}
  def get_ga4_report(%Plausible.Google.GA4.ReportRequest{} = report_request) do
    params = %{
      requests: [
        %{
          ...
          metrics: Enum.map(report_request.metrics, &build_ga4_metric/1),
          orderBys: [
            %{
              dimension: %{
                dimensionName: "date",
    ...
    url =
      "#{reporting_ga4_api_url()}/v1beta/#{report_request.property}:batchRunReports"
    ...
    with {:ok, %{body: body}} <- response,
         ...
         {:ok, report} <- convert_to_ga4_maps(report) do
      {:ok, {report, row_count}}
    else
      {:error, %{reason: %{status: status, body: body}}} ->
        Sentry.Context.set_extra_context(%{ga_response: %{body: body, status: status}})
  ...
  defp build_ga4_metric(expression) do
  ...
  defp convert_to_ga4_maps(response) do
    Logger.error(
      "Google Analytics: Failed to read report in response. Reason: #{inspect(response)}"
    )
  ...
  def list_accounts_for_user(access_token) do
    url = "#{admin_ga4_api_url()}/v1beta/accountSummaries"
  ...
  @earliest_ga4_valid_date "2015-08-14"
  def get_analytics_ga4_start_date(property, access_token) do
    params = %{
      requests: [
        %{
          property: "#{property}",
          dateRanges: [
            %{startDate: @earliest_ga4_valid_date, endDate: Date.to_iso8601(Timex.today())}
          ],
    ...
    url = "#{reporting_ga4_api_url()}/v1beta/#{property}:batchRunReports"
    headers = [{"Authorization", "Bearer #{access_token}"}]
  ...
  defp reporting_ga4_api_url, do: "https://analyticsdata.googleapis.com"
  defp admin_ga4_api_url, do: "https://analyticsadmin.googleapis.com"
