defmodule PlausibleWeb.Plugs.AuthorizePublicAPI do
  ...
  defp verify_by_scope(conn, api_key, "stats:read:" <> _ = scope) do
    with :ok <- check_scope(api_key, scope),
         ...
         :ok <- verify_site_access(conn, api_key, site, Plausible.Billing.Feature.StatsAPI) do
  ...
  defp maybe_verify_site_access(conn, api_key, feature) do
    case find_site(conn.params["site_id"]) do
      {:ok, site} ->
        verify_site_access(conn, api_key, site, feature)
  ...
  defp verify_site_access(conn, api_key, site, feature) do
    ...
    cond do
      Plausible.Sites.consolidated?(site) && !conn.assigns[:allow_consolidated_views] ->
