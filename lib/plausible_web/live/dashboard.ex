defmodule PlausibleWeb.DashboardLive do
  ...
  def mount(%{"domain" => site_domain}, session, socket) do
    socket =
      if connected?(socket) do
        ...
        assign(socket, site_domain: site_domain, graph: nil, top_stats: nil, pages: nil)
      end
  ...
  def handle_info(:fetch_graph, socket) do
    query = Query.from(socket.assigns[:site].timezone, %{"period" => "12mo"})
  ...
  def handle_info(:fetch_top_stats, socket) do
    ...
    query = Query.from(site.timezone, %{"period" => "12mo"})
