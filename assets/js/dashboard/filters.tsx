...
function filterText(key, rawValue, query) {
  ...
  if (key === 'props') {
    ...
    return (
      <>
        {eventName}.{metaKey} {toFilterType(metaValue)}{' '}
  ...
  if (key === 'browser_version') {
    ...
    return (
      <>
        {browserName}.Version {type} <b>{value}</b>
      </>
    )
  }
  if (key === 'os_version') {
  ...
  if (formattedFilter) {
    return (
      <>
        {formattedFilter} {type} <b>{value}</b>
      </>
    )
  }
  ...
  return (
    <Menu.Item key={option}>
      {({ active }) => (
        <Link
          to={{
            pathname: `/${encodeURIComponent(site.domain)}/filter/${option}`,
            search: window.location.search
          }}
...
function DropdownContent({ history, site, query, wrapped }) {
  ...
  if (wrapped === 0 || addingFilter) {
    return Object.keys(FILTER_GROUPS)
      .filter((option) =>
        option === 'props' ? site.flags.custom_dimension_filter : true
      )
  ...
  return (
    <>
      <div
        className="border-b border-gray-200 dark:border-gray-500 px-4 sm:py-2 py-3 text-sm leading-tight hover:text-indigo-700 dark:hover:text-indigo-500 hover:cursor-pointer"
        onClick={() => setAddingFilter(true)}
      >
        + Add filter
      </div>
      {appliedFilters(query).map((filter) =>
        renderDropdownFilter(site, history, filter, query)
      )}
      <Menu.Item key="clear">
        <div
...
class Filters extends React.Component {
  ...
  renderDropdownButton() {
    ...
    return (
      <>
        <SearchIcon
          className="-ml-1 mr-1 h-4 w-4 md:h-4 md:w-4"
          aria-hidden="true"
        />
