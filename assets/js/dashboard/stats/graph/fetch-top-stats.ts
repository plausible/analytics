...
export function topStatsQueries(
  ...
  if (isRealTimeDashboard(dashboardState)) {
    currentVisitorsQuery = createStatsQuery(
...
function constructTopStatsQuery(
  ...
  const adjustedDashboardState = { ...dashboardState }

  if (
    !isComparisonEnabled(dashboardState.comparison) &&
    !isRealTimeDashboard(dashboardState)
  ) {
    adjustedDashboardState.comparison = ComparisonMode.previous_period
  }

  if (isRealTimeDashboard(dashboardState)) {
    adjustedDashboardState.period = DashboardPeriod.realtime_30m
  }
