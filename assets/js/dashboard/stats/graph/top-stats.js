...
export default class TopStats extends React.Component {
    ...
    return (
      <div>
        ...
        {this.canMetricBeGraphed(stat) && (
          <div className="font-normal text-xs">{this.titleFor(stat)}</div>
        )}
      </div>
    )
  }
  ...
  canMetricBeGraphed(stat) {
    const isTotalUniqueVisitors =
      this.props.query.filters.goal && stat.name === 'Unique visitors'
  ...
  renderStat(stat) {
    return (
      <Tooltip
        info={this.topStatTooltip(stat)}
        className="flex items-center justify-between my-1 whitespace-nowrap"
      >
        <b className="mr-4 text-xl md:text-2xl dark:text-gray-100">
  ...
  render() {
    ...
    const stats =
      topStatData &&
      topStatData.top_stats.map((stat, index) => {
        ...
        return (
          ...
          >
            <div
              className={`text-xs font-bold tracking-wide text-gray-500 uppercase dark:text-gray-400 whitespace-nowrap flex w-content border-b ${
                isSelected
