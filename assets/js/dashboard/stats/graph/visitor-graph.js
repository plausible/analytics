...
import { hasConversionGoalFilter, hasPageFilter, isRealTimeDashboard } from '../../util/filters'
...
async function fetchTopStats(site, query) {
  ...
  if (isRealTimeDashboard(query) && hasConversionGoalFilter(query)) {
    topStatsQuery = {...q, metrics: ['visitors', 'events'], period: 'realtime_30m'}
  } else if (isRealTimeDashboard(query)) {
    topStatsQuery = {...q, metrics: ['visitors', 'pageviews'], period: 'realtime_30m'}
    ...
    topStatsQuery = {...q, metrics: ['visitors', 'visits', 'pageviews', 'scroll_depth']}
  } else if (hasPageFilter(query)) {
    topStatsQuery = {...q, metrics: ['visitors', 'visits', 'pageviews', 'bounce_rate', 'scroll_depth', 'time_on_page']}
  } else {
    topStatsQuery = {...q, metrics: ['visitors', 'visits', 'pageviews', 'views_per_visit', 'bounce_rate', 'visit_duration']}
  }
  ...
  const currentVisitorsResponse =
    isRealTimeDashboard(query)
      ? await api.stats(site, {...q, metrics: ['visitors'], filters: [], dimensions: []})
      : null
...
export default function VisitorGraph({ updateImportedDataInView }) {
  ...
  async function fetchTopStatsAndGraphData() {
    ...
    const availableMetrics = topStats.filter(stat => stat.graphable).map(stat => stat.metric)
  ...
  function importedSwitchVisible() {
    return topStatData && !['no_imported_data', 'out_of_range'].includes(topStatData.meta.imports_skip_reason)
  }

  function importedSwitchDisabled() {
    return topStatData && topStatData.meta.imports_skip_reason === 'unsupported_query'
  }

  function importedSwitchTooltip() {
    if (!topStatData) { return }
