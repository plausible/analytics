...
export default function ListReport(props) {
  ...
  function fetchData() {
    if (
      typeof prevQuery.current === 'undefined' ||
      prevQuery.current !== props.query
    ) {
      prevQuery.current = props.query
      setState({ loading: true, list: null })
      props.fetchData().then((res) => setState({ loading: false, list: res }))
    }
  }
      ...
      >
        ...
        >
          <span
            ...
            tooltip={props.tooltipText && props.tooltipText(listItem)}
          >
            <Link
            ...
            <ExternalLink
              item={listItem}
              externalLinkDest={props.externalLinkDest}
            />
          </span>
        </Bar>
        <span
          className="font-medium dark:text-gray-200 w-20 text-right"
          tooltip={listItem[valueKey]}
        >
          {numberFormatter(listItem[valueKey])}
          {listItem.percentage >= 0 ? (
            <span className="inline-block w-8 pl-1 text-xs text-right">
              ({listItem.percentage}%)
            </span>
          ) : null}
        </span>
  ...
  return (
    <LazyLoader onVisible={onVisible} className="flex flex-col flex-grow">
      ...
      {props.detailsLink && !state.loading && (
        <MoreLink url={props.detailsLink} list={state.list} />
      )}
    </LazyLoader>
  )
}
