    ...
    return (
      <div className="flex items-center justify-between my-1 text-sm" key={page.name}>
        <div className="w-full h-8 truncate" style={{maxWidth: 'calc(100% - 4rem)'}}>
          <Bar count={page.count} all={this.state.pages} bg="bg-orange-50 dark:bg-gray-700 dark:bg-opacity-50" />
          <span className="flex px-2 group" style={{marginTop: '-26px'}} >
            <Link to={{pathname: window.location.pathname, search: query.toString()}} className="block hover:underline dark:text-gray-400">{page.name}</Link>
        ...
        <span className="font-medium dark:text-gray-300">{numberFormatter(page.count)}</span>
      </div>
    )
  }
