...
class LineGraph extends React.Component {
          ...
          yAxes: [
            {
              ticks: {
                ...
                autoSkip: true,
                maxTicksLimit: 8,
                fontColor: this.props.darkTheme
                  ? 'rgb(243, 244, 246)'
                  : undefined,
              },
              gridLines: {
    ...
    const stats = this.props.graphData.top_stats.map((stat, index) => {
      ...
      return (
        <div className={`px-8 w-1/2 my-4 lg:w-auto ${border}`} key={stat.name}>
          <div className="text-gray-500 dark:text-gray-400 text-xs font-bold tracking-wide uppercase">
            {stat.name}
          </div>
          <div className="my-1 flex justify-between items-center">
            <b className="text-2xl mr-4 dark:text-gray-100">
  ...
  downloadLink() {
    ...
    )}/visitors.csv${api.serializeQuery(this.props.query)}`;

    return (
      <a href={endpoint} download>
        <svg
          className="feather w-4 h-5 absolute text-gray-700 dark:text-gray-300"
...
export default class VisitorGraph extends React.Component {
  ...
  componentDidMount() {
    this.fetchGraphData();
    if (this.props.timer)
      this.props.timer.onTick(this.fetchGraphData.bind(this));
  }
  ...
  render() {
    return (
      <div className="w-full relative bg-white dark:bg-gray-825 shadow-xl rounded mt-6 main-graph">
        {this.state.loading && (
          <div className="loading pt-24 sm:pt-32 md:pt-48 mx-auto">
            <div></div>
          </div>
        )}
        {this.renderInner()}
