...
export default class CurrentVisitors extends React.Component {
  ...
  render() {
    if (countFilters(this.props.query) !== 0) {
      return null;
    }
    ...
    if (currentVisitors !== null) {
      return (
        <Link
          to={`/${encodeURIComponent(this.props.site.domain)}?period=realtime`}
          className="block text-sm font-bold text-gray-500 dark:text-gray-300 mr-auto ml-2"
        >
          <svg
            className="w-2 mr-2 fill-current text-green-500 inline"
            viewBox="0 0 16 16"
