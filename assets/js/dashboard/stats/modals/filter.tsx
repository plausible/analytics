...
export function toFilterType(value) {
  return (
    Object.keys(FILTER_PREFIXES).find(
      (type) => FILTER_PREFIXES[type] === value[0]
    ) || FILTER_TYPES.is
  )
}

export function valueWithoutPrefix(value) {
  return [FILTER_TYPES.isNot, FILTER_TYPES.contains].includes(
    toFilterType(value)
  )
    ? value.substring(1)
    : value
...
class FilterModal extends React.Component {
  ...
  queryForSuggestions(query, formFilters, filter) {
    if (filter === 'prop_key') {
      const propsFilter = formFilters.prop_value
        ? { '': formFilters.prop_value }
        : null
      return { ...query, filters: { ...query.filters, props: propsFilter } }
    } else if (filter === 'prop_value') {
      const propsFilter = formFilters.prop_key
        ? { [formFilters.prop_key]: '!(none)' }
        : null
      return { ...query, filters: { ...query.filters, props: propsFilter } }
    } else {
      return {
    ...
    if (
      ...
      return (
        <p className="mt-6 text-xs text-gray-500">
          ...
          to group all of your blog posts. Or use double asterisks in front and
          back (e.g. **keyword**) to group all URLs containing a specific
          keyword.
        </p>
      )
    }
