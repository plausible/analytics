  ...
  return (
    <Menu.Item key={filterIndex}>
      <div className="px-3 md:px-4 sm:py-2 py-3 text-sm leading-tight flex items-center justify-between" key={filterIndex}>
        <Link
          title={`Edit filter: ${filter.displayName()}`}
          to={{ pathname: `/${encodeURIComponent(site.domain)}/filter/${filter.modalGroup()}`, search: window.location.search }}
        ...
        <b
          title={`Remove filter: ${filter.displayName()}`}
...
function DropdownContent({ history, site, query, wrapped }) {
  ...
  return (
    <>
      ...
      {query._filters.map((filter, index) => renderDropdownFilter(index, filter, site, history, query))}
      <Menu.Item key="clear">
...
class Filters extends React.Component {
  ...
  rewrapFilters() {
    ...
    if (this.props.query._filters.length > 0 && viewport <= 768) {
    ...
    if (wrapped !== 1 || !items || this.props.query._filters.length === 1) {
      return;
    };
    ...
    return (
      <span key={filterIndex} className="flex bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 shadow text-sm rounded mr-2 items-center">
        <Link
          ...
          to={{
            pathname: `/${encodeURIComponent(this.props.site.domain)}/filter/${filter.modalGroup()}`,
  ...
  renderDropdownButton() {
    if (this.state.wrapped === 2) {
      const filterCount = this.props.query._filters.length
      return (
        <>
    ...
    if (this.state.wrapped !== 2) {
      return (
        <div id="filters" className="flex flex-wrap">
          {query._filters.map((filter, index) => this.renderListFilter(index, filter, history, query))}
        </div>
      );
    }
