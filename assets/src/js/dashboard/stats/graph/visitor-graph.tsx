...
class LineGraph extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      metric &&
      graphData &&
      (graphData !== prevProps.graphData || darkTheme !== prevProps.darkTheme)
    ) {
      if (this.chart) {
        this.chart.destroy()
      }
  ...
  downloadLink() {
    if (this.props.query.period !== 'realtime') {
      if (this.state.exported) {
        ...
        )}/export${api.serializeQuery(this.props.query)}`
  ...
  samplingNotice() {
    const samplePercent =
      this.props.topStatData && this.props.topStatData.sample_percent
  ...
  render() {
    ...
    const extraClass =
      this.props.graphData && this.props.graphData.interval === 'hour'
        ? ''
        : 'cursor-pointer'
...
export default class VisitorGraph extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const topStatLabels =
      topStatData &&
      topStatData.top_stats
        .map(({ name }) => METRIC_MAPPING[name])
        .filter((name) => name)
    const prevTopStatLabels =
      prevState.topStatData &&
      prevState.topStatData.top_stats
        .map(({ name }) => METRIC_MAPPING[name])
        .filter((name) => name)
  ...
  fetchTopStatData() {
    api
      .get(
        `/api/stats/${encodeURIComponent(this.props.site.domain)}/top-stats`,
        this.props.query
      )
      .then((res) => {
        this.setState((state) => ({
  ...
  render() {
    ...
    return (
      <LazyLoader onVisible={this.onVisible}>
        <div
          className={`relative w-full mt-2 bg-white rounded shadow-xl dark:bg-gray-825 transition-padding ease-in-out duration-150 ${
            metric ? 'main-graph' : 'top-stats-only'
          }`}
        >
